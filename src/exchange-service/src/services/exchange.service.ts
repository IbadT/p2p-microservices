...
// import { ExchangeType, TransactionStatus, PaymentMethod, OfferStatus } from '@prisma/client';
...
//           status: {
//             in: [
//               TransactionStatus.PENDING_OFFER,
...
//       const status = data.action === RespondAction.ACCEPT ? OfferStatus.ACCEPTED : OfferStatus.DECLINED;
...
//         status: this.convertOfferStatusToTransactionStatus(status),
//         message: status === OfferStatus.ACCEPTED ? 'Offer accepted' : 'Offer declined',
//       };
//     } catch (error) {
...
//       case TransactionStatus.PAYMENT_SENT:
//         return ProtoTransactionStatus.PAYMENT_CONFIRMED;
...
//         return TransactionStatus.PENDING_OFFER;
...
//     if (hoursSinceUpdate < 24 && transaction.status !== 'DISPUTED') {

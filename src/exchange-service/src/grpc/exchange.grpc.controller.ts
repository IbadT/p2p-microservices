...
import {
  ...
  ExchangeType,
  RespondAction,
  UpdateMissedOffersRequest,
  GetExchangerStatusRequest,
  UnfreezeExchangerRequest,
  ExchangerStatus
...
export class ExchangeGrpcController {
  ...
  @Roles(UserRole.EXCHANGER)
  async getExchangerStatus(data: GetExchangerStatusRequest): Promise<ExchangerStatus> {
    try {
      const user = await this.exchangeService.getUser(data.exchangerId);
      return {
        exchangerId: user.id,
      ...
      this.logger.error(`Failed to get exchanger status: ${error.message}`, error.stack);
      throw new GrpcError(error.message, 'INTERNAL');
    }
  }
  ...
  @Roles(UserRole.MODERATOR, UserRole.ADMIN)
  async unfreezeExchanger(data: UnfreezeExchangerRequest): Promise<ExchangerStatus> {
    try {
      const user = await this.exchangeService.getUser(data.exchangerId);
      if (!user.isFrozen) {
        throw new GrpcError('Exchanger is not frozen', 'INVALID_ARGUMENT');
      }

      await this.exchangeService.unfreezeExchanger(data.exchangerId);
      ...
      this.logger.error(`Failed to unfreeze exchanger: ${error.message}`, error.stack);
      throw new GrpcError(error.message, 'INTERNAL');
    }
  }
} 